...
df.rename(columns = { '統計起始日': 'date', '上映日期': 'release', '上映院數': 'theaters', '中文片名': 'name', '國別地區': 'country', '累計銷售票數': 'tickets', '累計銷售金額': 'revenue' }, inplace = True)
df['date'] = pd.to_datetime(df['date'])
df['release'] = pd.to_datetime(df['release'])
df['tickets'] = pd.to_numeric(df['tickets'].str.replace(',', '')).astype('Int64')
df['revenue'] = pd.to_numeric(df['revenue'].str.replace(',', '')).astype('Float64')
df['revenue_10k'] = df['revenue'].div(10000)
null_index = df.index[(df['tickets'].isnull()) | (df['revenue_10k'].isnull())]
df = df.drop(null_index)

source = ColumnDataSource(data = dict(date = [], release = [], theaters = [], name = [], country = [], tickets = [], revenue = [], revenue_10k = []))
...
# widget
# todo: add filters
ticket_slider = Slider(start = 0, end = max(df['tickets']), value = 0, step = 100000, title = 'Tickets sold greater than')
...
plot = figure(width = 850, height = 280, tooltips = tooltips, toolbar_location = None, sizing_mode = 'scale_both', title = '')
plot.circle(x = 'tickets', y = 'revenue', source = source, size = 5)

# data table
columns = [
    TableColumn(field = 'date', title = 'Statistic date', formatter = DateFormatter()),
    TableColumn(field = 'release', title = 'Released date', formatter = DateFormatter()),
...
datatable = DataTable(width = 850, height = 280, source = source, columns = columns, sortable = True)
...
layout = column(row(index), row(filters, results), row(footer), sizing_mode = 'scale_both')
callback()
